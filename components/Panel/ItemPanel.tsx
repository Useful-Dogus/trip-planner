...
export default function ItemPanel({ item, isOpen, onClose, onSave, onDelete }: ItemPanelProps) {
  ...
  async function handleInlineUpdate(field: 'name' | 'budget', value: string) {
    if (!displayItem) return
    const trimmed = value.trim()
    if (field === 'name') {
      if (!trimmed || trimmed === displayItem.name) return
      await updateItem(displayItem.id, { name: trimmed })
    } else {
      const parsed = trimmed ? parseInt(trimmed) : null
      if (parsed === (displayItem.budget ?? null)) return
      await updateItem(displayItem.id, { budget: parsed })
    }
    onSave()
  }
...
function ItemDetailView({
  ...
  onInlineUpdate: (field: 'name' | 'budget', value: string) => void
}) {
  const scheduleRows = buildScheduleRows(item)
  const [editingName, setEditingName] = useState(false)
  const [nameValue, setNameValue] = useState(item.name)
  const [editingBudget, setEditingBudget] = useState(false)
  const [budgetValue, setBudgetValue] = useState(item.budget?.toString() ?? '')
  ...
  function commitName() {
    onInlineUpdate('name', nameValue)
    setEditingName(false)
  }

  function commitBudget() {
    onInlineUpdate('budget', budgetValue)
  ...
  return (
    <div className="px-5 py-4 space-y-5 pb-8 overflow-y-auto">
              ...
              {editingBudget ? (
                <input
                  ...
                  onKeyDown={e => {
                    ...
                    if (e.key === 'Escape') { setBudgetValue(item.budget?.toString() ?? ''); setEditingBudget(false) }
                  }}
                  className="text-sm font-medium text-gray-900 w-28 text-right border-b border-gray-400 outline-none bg-transparent"
                />
              ) : (
                <span
      ...
      {/* 예산이 없을 때 — 섹션 외부에서 인라인 추가 */}
      {item.budget === undefined && scheduleRows.length === 0 && (
        <section>
          ...
          {editingBudget ? (
            <input
              ...
              onKeyDown={e => {
                if (e.key === 'Enter') commitBudget()
                if (e.key === 'Escape') { setBudgetValue(''); setEditingBudget(false) }
              }}
              className="text-sm font-medium text-gray-900 border-b border-gray-400 outline-none bg-transparent w-28"
              placeholder="0"
              autoFocus
            />
          ) : (
            <span
              className={`text-sm cursor-text ${PLACEHOLDER_TONE}`}
              onClick={() => setEditingBudget(true)}
            >
              탭하여 예산 입력
            </span>
